...
export default function App() {
  ...
  const toggleDone = async (o) => {
    await updateDoc(doc(db, "orders", o.id), { zavrseno: !o.zavrseno });
    loadOrders();
  };
          ...
          <div style={{ background: "white", padding: 20, width: "90%", maxWidth: 400 }}>
            ...
            {selectedOrder.boje && (
              <>
                <p><b>Boje:</b></p>
                <ul>
                  {selectedOrder.boje.map((b, i) => <li key={i}>{b.boja} - {b.kolicina}</li>)}
                </ul>
              </>
            )}
            {selectedOrder.tekstil && (
              <>
                <p><b>Tekstil:</b></p>
                <ul>
                  {selectedOrder.tekstil.map((t, i) => <li key={i}>{t.velicina} - {t.kolicina} - {t.boja}</li>)}
                </ul>
              </>
            )}
